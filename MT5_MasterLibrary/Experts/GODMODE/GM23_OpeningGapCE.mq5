//+------------------------------------------------------------------+
//|                                    GM23_OpeningGapCE.mq5         |
//|                      GODMODE Master EA Library  -  Archetype 23   |
//|  ICT New-Day/New-Week Opening Gap (NDOG/NWOG): trade the reaction |
//|  at the gap midpoint (consequent encroachment) in gap direction.  |
//+------------------------------------------------------------------+
...
input group "=== Signals ==="
input double InpMinGapMult       = 0.5;
...
int OnInit()
  {
   ...
   g_log.Init("GODMODE_GM23_OpeningGapCE_log.csv");
   g_atrHandle = iATR(_Symbol, PERIOD_CURRENT, InpATRPeriod);
   Print("GM23 Opening Gap CE initialised. RiskMode=", EnumToString(InpRiskMode));
...
void OnTick()
  {
   ...
   if(g_guards.DailyDDBreached(InpMaxDailyDDPct))
     { g_trade.CloseAll(); g_guards.Halt(); g_log.Log("GM23","KILL_DD",GM_NONE,0,0,0,0,0,0,"daily DD"); return; }
   ...
   if(MathAbs(gap) < InpMinGapMult*atr) return;
   ...
   if(gap>0 && lo1<=ce && close>=ce)      { dir=GM_LONG;  sl=MathMin(yClose,lo1)-pad; }
   else if(gap<0 && hi1>=ce && close<=ce) { dir=GM_SHORT; sl=MathMax(yClose,hi1)+pad; }
   ...
   if(lots<=0.0) { g_log.Log("GM23","SKIP_SIZE",dir,entry,sl,tp,0,InpRR,0,"min lot risk too high"); return; }

   if(g_trade.Open(dir, lots, sl, tp, "GM23_CE"))
     { g_tradesToday++; g_log.Log("GM23","ENTRY",dir,entry,sl,tp,lots,InpRR,1,"opening-gap CE react"); }
  }
//+------------------------------------------------------------------+
