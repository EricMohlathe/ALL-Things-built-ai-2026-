//+------------------------------------------------------------------+
//|                                    GM13_PriceAction.mq5           |
//|                      GODMODE Master EA Library  -  Archetype 13   |
//|                                                                  |
//|  Classic price-action trigger candles (Nial Fuller set):         |
//|  pin bar, engulfing, inside-bar fakey. Distilled from the         |
//|  Price Action / pin-bar / inside-bar / fakey / false-break PDFs.  |
//|                                                                  |
//|  Logic: with-trend (EMA) reversal/continuation triggers -         |
//|  bullish pin or bullish engulfing above the EMA = long; mirror    |
//|  for short. SL beyond the trigger candle; TP = fixed R.           |
//+------------------------------------------------------------------+
...
input group "=== Signals ==="
input bool   InpUsePin           = true;        // pin bar
input bool   InpUseEngulf        = true;        // engulfing
...
int OnInit()
  {
   ...
   g_log.Init("GODMODE_GM13_PriceAction_log.csv");
   g_atrHandle = iATR(_Symbol, PERIOD_CURRENT, InpATRPeriod);
   g_emaHandle = iMA(_Symbol, PERIOD_CURRENT, InpEMAPeriod, 0, MODE_EMA, PRICE_CLOSE);
   Print("GM13 Price Action initialised. RiskMode=", EnumToString(InpRiskMode));
...
void OnTick()
  {
   ...
   if(g_guards.DailyDDBreached(InpMaxDailyDDPct))
     { g_trade.CloseAll(); g_guards.Halt(); g_log.Log("GM13","KILL_DD",GM_NONE,0,0,0,0,0,0,"daily DD"); return; }
   ...
   bool bull = (InpUsePin && GM_IsBullPin(1)) || (InpUseEngulf && GM_IsBullEngulf());
   bool bear = (InpUsePin && GM_IsBearPin(1)) || (InpUseEngulf && GM_IsBearEngulf());

   GM_Dir dir=GM_NONE; double sl=0.0;
   if(bull && (!InpUseEMABias || close>ema)) { dir=GM_LONG;  sl=iLow(_Symbol,PERIOD_CURRENT,1)-pad; }
   else if(bear && (!InpUseEMABias || close<ema)) { dir=GM_SHORT; sl=iHigh(_Symbol,PERIOD_CURRENT,1)+pad; }
   ...
   if(lots<=0.0) { g_log.Log("GM13","SKIP_SIZE",dir,entry,sl,tp,0,InpRR,0,"min lot risk too high"); return; }

   if(g_trade.Open(dir, lots, sl, tp, "GM13_PA"))
     { g_tradesToday++; g_log.Log("GM13","ENTRY",dir,entry,sl,tp,lots,InpRR,1,"PA trigger candle"); }
  }
//+------------------------------------------------------------------+
