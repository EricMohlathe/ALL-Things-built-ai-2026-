//+------------------------------------------------------------------+
//|                                    GM19_RSI2MeanReversion.mq5    |
//|                      GODMODE Master EA Library  -  Archetype 19   |
//|  Connors RSI(2) pullback inside a long-EMA trend filter:          |
//|  buy deep oversold above the EMA, sell deep overbought below it.  |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   g_log.Init("GODMODE_GM19_RSI2MeanReversion_log.csv");
   g_atrHandle = iATR(_Symbol, PERIOD_CURRENT, InpATRPeriod);
   g_emaHandle = iMA(_Symbol, PERIOD_CURRENT, InpTrendEMA, 0, MODE_EMA, PRICE_CLOSE);
   g_rsiHandle = iRSI(_Symbol, PERIOD_CURRENT, InpRSIPeriod, PRICE_CLOSE);
   Print("GM19 RSI(2) mean reversion initialised. RiskMode=", EnumToString(InpRiskMode));
...
double RSIValue() { double b[]; if(g_rsiHandle==INVALID_HANDLE||CopyBuffer(g_rsiHandle,0,1,1,b)<=0) return 50.0; return b[0]; }
...
void OnTick()
  {
   ...
   if(g_guards.DailyDDBreached(InpMaxDailyDDPct))
     { g_trade.CloseAll(); g_guards.Halt(); g_log.Log("GM19","KILL_DD",GM_NONE,0,0,0,0,0,0,"daily DD"); return; }
   ...
   if(close>ema && rsi<InpOversold)        { dir=GM_LONG;  sl=close-InpSLAtrMult*atr; }
   else if(close<ema && rsi>InpOverbought) { dir=GM_SHORT; sl=close+InpSLAtrMult*atr; }
   ...
   if(lots<=0.0) { g_log.Log("GM19","SKIP_SIZE",dir,entry,sl,tp,0,InpRR,0,"min lot risk too high"); return; }

   if(g_trade.Open(dir, lots, sl, tp, "GM19_RSI2"))
     { g_tradesToday++; g_log.Log("GM19","ENTRY",dir,entry,sl,tp,lots,InpRR,1,"RSI2 pullback in trend"); }
  }
//+------------------------------------------------------------------+
