//+------------------------------------------------------------------+
//|                                    GM22_EMAPullback.mq5          |
//|                      GODMODE Master EA Library  -  Archetype 22   |
//|  Trend by EMA stack; enter on the pullback-and-reclaim of the     |
//|  fast EMA (Rayner Teo / Adam Khoo trend-continuation style).      |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   g_log.Init("GODMODE_GM22_EMAPullback_log.csv");
   g_atrHandle  = iATR(_Symbol, PERIOD_CURRENT, InpATRPeriod);
   g_fastHandle = iMA(_Symbol, PERIOD_CURRENT, InpFastEMA, 0, MODE_EMA, PRICE_CLOSE);
   g_slowHandle = iMA(_Symbol, PERIOD_CURRENT, InpSlowEMA, 0, MODE_EMA, PRICE_CLOSE);
   Print("GM22 EMA Pullback initialised. RiskMode=", EnumToString(InpRiskMode));
   return INIT_SUCCEEDED;
  }
void OnDeinit(const int reason) { }

double ATRValue()  { double b[]; if(g_atrHandle==INVALID_HANDLE||CopyBuffer(g_atrHandle,0,1,1,b)<=0) return 0.0; return b[0]; }
double FastValue() { double b[]; if(g_fastHandle==INVALID_HANDLE||CopyBuffer(g_fastHandle,0,1,1,b)<=0) return 0.0; return b[0]; }
double SlowValue() { double b[]; if(g_slowHandle==INVALID_HANDLE||CopyBuffer(g_slowHandle,0,1,1,b)<=0) return 0.0; return b[0]; }
...
void OnTick()
  {
   ...
   if(g_guards.DailyDDBreached(InpMaxDailyDDPct))
     { g_trade.CloseAll(); g_guards.Halt(); g_log.Log("GM22","KILL_DD",GM_NONE,0,0,0,0,0,0,"daily DD"); return; }
   ...
   double f=FastValue(), s=SlowValue();
   double pad=InpSLPaddingPoints*SymbolInfoDouble(_Symbol,SYMBOL_POINT);
   GM_Dir dir=GM_NONE; double sl=0.0;

   if(f>s && c1>s && lo1<=f && c1>f && c1>o1)      { dir=GM_LONG;  sl=lo1-pad; }
   else if(f<s && c1<s && hi1>=f && c1<f && c1<o1) { dir=GM_SHORT; sl=hi1+pad; }
   ...
   if(lots<=0.0) { g_log.Log("GM22","SKIP_SIZE",dir,entry,sl,tp,0,InpRR,0,"min lot risk too high"); return; }

   if(g_trade.Open(dir, lots, sl, tp, "GM22_EMApb"))
     { g_tradesToday++; g_log.Log("GM22","ENTRY",dir,entry,sl,tp,lots,InpRR,1,"EMA pullback reclaim"); }
  }
//+------------------------------------------------------------------+
