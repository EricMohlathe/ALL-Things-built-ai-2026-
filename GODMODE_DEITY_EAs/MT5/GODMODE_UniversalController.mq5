...
int      gAtr[], gSma50[], gSma200[], gRsi[], gTrendN[], gTrendMA[], gEntryBar[], gBarCount[];
long     gMagic[];
datetime gLastBar[];
int      gN = 0;
...
int OnInit()
{
   ...
   for(int i=0;i<ns;i++)
   {
      ...
      int tn=(ArraySize(f)>=6)?(int)StringToInteger(f[5]):0;
      int hT=(tn>0)?iMA(sym,PERIOD_D1,tn,0,MODE_SMA,PRICE_CLOSE):INVALID_HANDLE;
      int k=gN; gN++;
      ArrayResize(gSym,gN);ArrayResize(gSetup,gN);ArrayResize(gStop,gN);ArrayResize(gTp,gN);ArrayResize(gTrail,gN);
      ArrayResize(gAtr,gN);ArrayResize(gSma50,gN);ArrayResize(gSma200,gN);ArrayResize(gRsi,gN);ArrayResize(gTrendN,gN);ArrayResize(gTrendMA,gN);
      ...
      gAtr[k]=hA; gSma50[k]=h50; gSma200[k]=h200; gRsi[k]=hR; gTrendN[k]=tn; gTrendMA[k]=hT;
...
void OnDeinit(const int r){ for(int i=0;i<gN;i++){ IndicatorRelease(gAtr[i]);
   ...
   if(gTrendMA[i]!=INVALID_HANDLE) IndicatorRelease(gTrendMA[i]); } }

int TrendGate(int i,int sig){ if(sig==0||gTrendMA[i]==INVALID_HANDLE) return sig;
   double c=iClose(gSym[i],PERIOD_D1,1), m=Bf(gTrendMA[i],1); if(m==EMPTY_VALUE) return sig;
   if(sig>0 && c<=m) return 0; if(sig<0 && c>=m) return 0; return sig; }
...
void OnTick(){
   ...
   for(int i=0;i<gN;i++){
      ...
      if(t!=gLastBar[i]){ gLastBar[i]=t; gBarCount[i]++; Manage(i);
         int dir=CurDir(i), sig=TrendGate(i,Signal(i));
