// GM04 Order Block return — GODMODE Master cBot Library (cTrader)
// ICT/SMC order-block educators. Mirror of MT5 GM04.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM04_OrderBlockReturn : Robot
    {
        ...
        [Parameter("OB Lookback", Group = "Entry", DefaultValue = 30)] public int OBLookback { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM04"); _guards = new GMGuards(this, "GM04");
            _log = new GMLogger("GODMODE_GM04_OB_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, EMAPeriod);
            Print("GM04 Order Block Return started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM04", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            GMDir dir = GMDir.None; double sl = 0; double obTop, obBot;

            if (GMStruct.FindBullishOB(Bars, OBLookback, atr, DispMult, out obTop, out obBot))
            {
                if (price <= obTop && price >= obBot && (!UseEMA || price > ema)) { dir = GMDir.Long; sl = obBot - pad; }
            }
            if (dir == GMDir.None && GMStruct.FindBearishOB(Bars, OBLookback, atr, DispMult, out obTop, out obBot))
            {
                if (price <= obTop && price >= obBot && (!UseEMA || price < ema)) { dir = GMDir.Short; sl = obTop + pad; }
            }
            ...
            if (vol <= 0) { _log.Log("GM04", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM04_OB"))
            { _tradesToday++; _log.Log("GM04", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "OB return"); }
        }
    }
}
