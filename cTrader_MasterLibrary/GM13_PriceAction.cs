// GM13 Price Action trigger candles — GODMODE Master cBot Library (cTrader)
// Nial Fuller pin / engulfing / inside-bar set. Mirror of MT5 GM13.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM13_PriceAction : Robot
    {
        ...
        [Parameter("Use Pin", Group = "Signals", DefaultValue = true)] public bool UsePin { get; set; }
        [Parameter("Use Engulf", Group = "Signals", DefaultValue = true)] public bool UseEngulf { get; set; }
        [Parameter("Use EMA Bias", Group = "Signals", DefaultValue = true)] public bool UseEMA { get; set; }
        [Parameter("EMA Period", Group = "Signals", DefaultValue = 50)] public int EMAPeriod { get; set; }
        [Parameter("SL Padding Pips", Group = "Signals", DefaultValue = 1.0)] public double SLPadPips { get; set; }
        [Parameter("Reward:Risk", Group = "Signals", DefaultValue = 2.0)] public double RR { get; set; }
        [Parameter("ATR Period", Group = "Signals", DefaultValue = 14)] public int ATRPeriod { get; set; }
        [Parameter("Max Trades/Day", Group = "Signals", DefaultValue = 3)] public int MaxTrades { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM13"); _guards = new GMGuards(this, "GM13");
            _log = new GMLogger("GODMODE_GM13_PriceAction_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, EMAPeriod);
            Print("GM13 Price Action started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM13", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            bool bull = (UsePin && GMStruct.IsBullPin(Bars, 1)) || (UseEngulf && GMStruct.IsBullEngulf(Bars));
            bool bear = (UsePin && GMStruct.IsBearPin(Bars, 1)) || (UseEngulf && GMStruct.IsBearEngulf(Bars));

            GMDir dir = GMDir.None; double sl = 0;
            if (bull && (!UseEMA || close > ema)) { dir = GMDir.Long; sl = Bars.LowPrices.Last(1) - pad; }
            else if (bear && (!UseEMA || close < ema)) { dir = GMDir.Short; sl = Bars.HighPrices.Last(1) + pad; }
            ...
            if (vol <= 0) { _log.Log("GM13", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM13_PA"))
            { _tradesToday++; _log.Log("GM13", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "PA trigger candle"); }
        }
    }
}
