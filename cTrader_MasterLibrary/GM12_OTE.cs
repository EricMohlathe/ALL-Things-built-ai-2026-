// GM12 ICT Optimal Trade Entry (0.62-0.79 Fib) — GODMODE Master cBot Library (cTrader)
// HowToTrade OTE guide. Mirror of MT5 GM12.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM12_OTE : Robot
    {
        ...
        [Parameter("Swing Lookback", Group = "Entry", DefaultValue = 20)] public int SwingLB { get; set; }
        ...
        [Parameter("Require FVG", Group = "Entry", DefaultValue = false)] public bool RequireFVG { get; set; }
        [Parameter("FVG Lookback", Group = "Entry", DefaultValue = 15)] public int FVGLB { get; set; }
        [Parameter("SL Padding Pips", Group = "Entry", DefaultValue = 1.0)] public double SLPadPips { get; set; }
        [Parameter("TP Type 0=Origin 1=RR", Group = "Entry", DefaultValue = 0)] public int TPType { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM12"); _guards = new GMGuards(this, "GM12");
            _log = new GMLogger("GODMODE_GM12_OTE_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, EMAPeriod);
            Print("GM12 OTE started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM12", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double swingHi = GMStruct.HighestHigh(Bars, SwingLB, 1), swingLo = GMStruct.LowestLow(Bars, SwingLB, 1);
            GMDir dir = GMDir.None; double zTop, zBot; double sl = 0, tp = 0; double fTop, fBot;

            if ((!UseEMA || price > ema) && GMStruct.OTELong(Bars, SwingLB, out zTop, out zBot))
            {
                bool inZone = price <= zTop && price >= zBot;
                bool fvgOK = !RequireFVG || (GMStruct.FindBullishFVG(Bars, FVGLB, out fTop, out fBot) && price <= fTop && price >= fBot);
                if (inZone && fvgOK) { dir = GMDir.Long; sl = swingLo - pad; tp = TPType == 1 ? 0 : swingHi; }
            }
            if (dir == GMDir.None && (!UseEMA || price < ema) && GMStruct.OTEShort(Bars, SwingLB, out zTop, out zBot))
            {
                bool inZone = price <= zTop && price >= zBot;
                bool fvgOK = !RequireFVG || (GMStruct.FindBearishFVG(Bars, FVGLB, out fTop, out fBot) && price <= fTop && price >= fBot);
                if (inZone && fvgOK) { dir = GMDir.Short; sl = swingHi + pad; tp = TPType == 1 ? 0 : swingLo; }
            }
            if (dir == GMDir.None) return;

            double entry = dir == GMDir.Long ? Symbol.Ask : Symbol.Bid;
            double risk = Math.Abs(entry - sl); if (risk <= 0) return;
            if (TPType == 1) tp = dir == GMDir.Long ? entry + risk * RR : entry - risk * RR;
            double rr = Math.Abs(tp - entry) / risk;
            if (rr < MinRR) { _log.Log("GM12", "SKIP_RR", dir, entry, sl, tp, 0, rr, 0, "below MinRR"); return; }

            double vol = GMUtil.CalcVolume(this, risk, RiskPct(), LotMult());
            if (vol <= 0) { _log.Log("GM12", "SKIP_SIZE", dir, entry, sl, tp, 0, rr, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM12_OTE"))
            { _tradesToday++; _log.Log("GM12", "ENTRY", dir, entry, sl, tp, vol, rr, 1, "OTE 0.62-0.79"); }
        }
    }
}
