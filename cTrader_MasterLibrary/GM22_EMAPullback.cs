// GM22 EMA pullback trend continuation — GODMODE Master cBot Library (cTrader)
// Trend by EMA stack, enter on the pullback-and-reclaim of the fast EMA. Mirror of MT5 GM22.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM22_EMAPullback : Robot
    {
        ...
        [Parameter("Fast EMA", Group = "Entry", DefaultValue = 20)] public int FastEMA { get; set; }
        [Parameter("Slow EMA", Group = "Entry", DefaultValue = 50)] public int SlowEMA { get; set; }
        ...
        [Parameter("Use Trail", Group = "Manage", DefaultValue = true)] public bool UseTrail { get; set; }
        ...
        private AverageTrueRange _atr; private ExponentialMovingAverage _fast, _slow; private int _tradesToday;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM22"); _guards = new GMGuards(this, "GM22");
            _log = new GMLogger("GODMODE_GM22_EMAPullback_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _fast = Indicators.ExponentialMovingAverage(Bars.ClosePrices, FastEMA);
            _slow = Indicators.ExponentialMovingAverage(Bars.ClosePrices, SlowEMA);
            Print("GM22 EMA Pullback started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM22", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double f = _fast.Result.Last(1), s = _slow.Result.Last(1);
            double pad = SLPadPips * Symbol.PipSize;
            GMDir dir = GMDir.None; double sl = 0;

            if (f > s && c1 > s && lo1 <= f && c1 > f && c1 > o1) { dir = GMDir.Long; sl = lo1 - pad; }
            else if (f < s && c1 < s && hi1 >= f && c1 < f && c1 < o1) { dir = GMDir.Short; sl = hi1 + pad; }
            ...
            if (vol <= 0) { _log.Log("GM22", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM22_EMApb"))
            { _tradesToday++; _log.Log("GM22", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "EMA pullback reclaim"); }
        }
    }
}
