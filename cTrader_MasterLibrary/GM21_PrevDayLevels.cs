// GM21 Previous-Day High/Low sweep — GODMODE Master cBot Library (cTrader)
// Liquidity raid of PDH/PDL then reclaim (stop-hunt fade). Mirror of MT5 GM21.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM21_PrevDayLevels : Robot
    {
        ...
        [Parameter("Use PDH/PDL", Group = "Entry", DefaultValue = true)] public bool UsePDHL { get; set; }
        [Parameter("Use Prev-Week H/L", Group = "Entry", DefaultValue = false)] public bool UsePWHL { get; set; }
        ...
        private AverageTrueRange _atr; private Bars _daily; private Bars _weekly; private int _tradesToday;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM21"); _guards = new GMGuards(this, "GM21");
            _log = new GMLogger("GODMODE_GM21_PrevDayLevels_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _daily = MarketData.GetBars(TimeFrame.Daily);
            _weekly = MarketData.GetBars(TimeFrame.Weekly);
            Print("GM21 Prev-Day Levels started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM21", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            if (_daily.ClosePrices.Count < 2) return;

            double hi1 = Bars.HighPrices.Last(1), lo1 = Bars.LowPrices.Last(1), close = Bars.ClosePrices.Last(1);
            double pad = SLPadPips * Symbol.PipSize;
            GMDir dir = GMDir.None; double sl = 0; string lvl = "";

            if (UsePDHL)
            {
                double pdh = _daily.HighPrices.Last(1), pdl = _daily.LowPrices.Last(1);
                if (hi1 > pdh && close < pdh) { dir = GMDir.Short; sl = hi1 + pad; lvl = "PDH"; }
                else if (lo1 < pdl && close > pdl) { dir = GMDir.Long; sl = lo1 - pad; lvl = "PDL"; }
            }
            if (dir == GMDir.None && UsePWHL && _weekly.ClosePrices.Count >= 2)
            {
                double pwh = _weekly.HighPrices.Last(1), pwl = _weekly.LowPrices.Last(1);
                if (hi1 > pwh && close < pwh) { dir = GMDir.Short; sl = hi1 + pad; lvl = "PWH"; }
                else if (lo1 < pwl && close > pwl) { dir = GMDir.Long; sl = lo1 - pad; lvl = "PWL"; }
            }
            ...
            if (vol <= 0) { _log.Log("GM21", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM21_" + lvl))
            { _tradesToday++; _log.Log("GM21", "ENTRY", dir, entry, sl, tp, vol, RR, 1, lvl + " sweep reclaim"); }
        }
    }
}
