// GM15 Supply & Demand zones — GODMODE Master cBot Library (cTrader)
// RBR/DBR demand, DBD/RBD supply: small-range base + displacement leave, trade the return. Mirror of MT5 GM15.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM15_SupplyDemand : Robot
    {
        ...
        [Parameter("Zone Lookback", Group = "Entry", DefaultValue = 30)] public int ZoneLB { get; set; }
        [Parameter("Displacement ATR Mult", Group = "Entry", DefaultValue = 1.2)] public double DispMult { get; set; }
        [Parameter("Base Max ATR Mult", Group = "Entry", DefaultValue = 0.6)] public double BaseMaxMult { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM15"); _guards = new GMGuards(this, "GM15");
            _log = new GMLogger("GODMODE_GM15_SupplyDemand_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, EMAPeriod);
            Print("GM15 Supply & Demand started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM15", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double close = Bars.ClosePrices.Last(1), lo1 = Bars.LowPrices.Last(1), hi1 = Bars.HighPrices.Last(1);
            double ema = _ema.Result.Last(1);
            double pad = SLPadPips * Symbol.PipSize;
            double zTop, zBot; GMDir dir = GMDir.None; double sl = 0;

            if ((!UseEMA || close > ema) && GMStruct.FindDemandBase(Bars, ZoneLB, atr, DispMult, BaseMaxMult, out zTop, out zBot)
                && lo1 <= zTop && close >= zBot)
            { dir = GMDir.Long; sl = zBot - pad; }
            else if ((!UseEMA || close < ema) && GMStruct.FindSupplyBase(Bars, ZoneLB, atr, DispMult, BaseMaxMult, out zTop, out zBot)
                && hi1 >= zBot && close <= zTop)
            { dir = GMDir.Short; sl = zTop + pad; }
            ...
            if (vol <= 0) { _log.Log("GM15", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM15_SD"))
            { _tradesToday++; _log.Log("GM15", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "S/D zone return"); }
        }
    }
}
