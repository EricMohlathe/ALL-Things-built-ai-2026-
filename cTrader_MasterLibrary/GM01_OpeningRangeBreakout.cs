// GM01 Opening Range Breakout — GODMODE Master cBot Library (cTrader)
// Casper/Ginger/JDUB/RP-Profits/ORB family. Mirror of MT5 GM01.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM01_OpeningRangeBreakout : Robot
    {
        ...
        [Parameter("OR Start Hour", Group = "Session", DefaultValue = 15)] public int ORStartHour { get; set; }
        [Parameter("OR Start Minute", Group = "Session", DefaultValue = 30)] public int ORStartMin { get; set; }
        [Parameter("OR Duration Min", Group = "Session", DefaultValue = 15)] public int ORDurMin { get; set; }
        [Parameter("Trade End Hour", Group = "Session", DefaultValue = 21)] public int TradeEndHour { get; set; }
        [Parameter("Force Close Hour", Group = "Session", DefaultValue = 22)] public int ForceCloseHour { get; set; }

        [Parameter("Reverse", Group = "Entry", DefaultValue = false)] public bool Reverse { get; set; }
        [Parameter("SL Type 0=OppRange 1=Candle 2=ATR", Group = "Entry", DefaultValue = 0)] public int SLType { get; set; }
        [Parameter("ATR Mult SL", Group = "Entry", DefaultValue = 2.0)] public double ATRMultSL { get; set; }
        ...
        private double _orHigh, _orLow; private bool _orReady, _tradedToday;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM01"); _guards = new GMGuards(this, "GM01");
            _log = new GMLogger("GODMODE_GM01_ORB_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM01 ORB started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            int orStart = ORStartHour * 60 + ORStartMin;
            int orEnd = orStart + ORDurMin;

            if (_guards.IsNewDay()) { _guards.OnNewDay(); _orHigh = 0; _orLow = 0; _orReady = false; _tradedToday = false; }
            if (_guards.IsHalted) return;
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM01", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            if (MaxConsec > 0 && _guards.TrailingLossStreak() >= MaxConsec) { _guards.Halt(); return; }
            if ((now.Hour + OffsetHours) % 24 >= ForceCloseHour) { _trade.CloseAll(); return; }

            double prevHigh = Bars.HighPrices.Last(1), prevLow = Bars.LowPrices.Last(1);
            bool inFormation = GMUtil.InWindow(mod, orStart, orEnd);
            if (inFormation)
            {
                if (_orHigh == 0 || prevHigh > _orHigh) _orHigh = prevHigh;
                if (_orLow == 0 || prevLow < _orLow) _orLow = prevLow;
            }
            else if (_orHigh > 0 && _orLow > 0 && !_orReady && mod >= orEnd)
            {
                _orReady = true;
                Chart.DrawHorizontalLine("GM01_ORH", _orHigh, Color.Lime);
                Chart.DrawHorizontalLine("GM01_ORL", _orLow, Color.Red);
            }

            if (!_orReady || _tradedToday || _trade.HasPosition) return;
            if ((now.Hour + OffsetHours) % 24 >= TradeEndHour) return;
            if (!_guards.SpreadOK(MaxSpread)) { _log.Log("GM01", "SKIP_SPREAD", GMDir.None, 0, 0, 0, 0, 0, 0, "spread"); return; }

            double close = Bars.ClosePrices.Last(1);
            double pad = SLPadPips * Symbol.PipSize;
            GMDir dir = GMDir.None;
            if (close > _orHigh) dir = Reverse ? GMDir.Short : GMDir.Long;
            if (close < _orLow) dir = Reverse ? GMDir.Long : GMDir.Short;
            if (dir == GMDir.None) return;

            double entry = dir == GMDir.Long ? Symbol.Ask : Symbol.Bid;
            double atr = Atr();
            double sl;
            if (SLType == 2 && atr > 0) sl = dir == GMDir.Long ? entry - atr * ATRMultSL : entry + atr * ATRMultSL;
            else if (SLType == 1) sl = dir == GMDir.Long ? Bars.LowPrices.Last(1) - pad : Bars.HighPrices.Last(1) + pad;
            else sl = dir == GMDir.Long ? _orLow - pad : _orHigh + pad;
            ...
            if (vol <= 0) { _log.Log("GM01", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM01_ORB"))
            { _tradedToday = true; _log.Log("GM01", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "ORB breakout"); }
        }
    }
}
