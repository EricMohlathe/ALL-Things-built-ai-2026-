// GM16 Market Structure BOS / CHoCH — GODMODE Master cBot Library (cTrader)
// Break of structure (continuation) / change of character (reversal) off fractal swings. Mirror of MT5 GM16.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM16_MarketStructure : Robot
    {
        ...
        [Parameter("Structure Lookback", Group = "Entry", DefaultValue = 40)] public int StructLB { get; set; }
        [Parameter("Swing Wing", Group = "Entry", DefaultValue = 2)] public int Wing { get; set; }
        [Parameter("Mode 0=BOS 1=CHoCH", Group = "Entry", DefaultValue = 0)] public int ModeP { get; set; }
        ...
        private double _prevSwingHi, _prevSwingLo; private bool _havePrevHi, _havePrevLo;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM16"); _guards = new GMGuards(this, "GM16");
            _log = new GMLogger("GODMODE_GM16_MarketStructure_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM16 Market Structure started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM16", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            bool haveHi = GMStruct.LastSwingHigh(Bars, StructLB, Wing, out shHi, out sH);
            bool haveLo = GMStruct.LastSwingLow(Bars, StructLB, Wing, out shLo, out sL);
            double close = Bars.ClosePrices.Last(1);

            // CHoCH = break against the prior structure leg; BOS = break in trend direction.
            bool brokeUp = haveHi && close > shHi;
            bool brokeDn = haveLo && close < shLo;
            bool upIsReversal = _havePrevHi && shHi < _prevSwingHi;   // breaking up after lower highs
            bool dnIsReversal = _havePrevLo && shLo > _prevSwingLo;   // breaking down after higher lows
            if (haveHi) { _prevSwingHi = shHi; _havePrevHi = true; }
            if (haveLo) { _prevSwingLo = shLo; _havePrevLo = true; }
            ...
            if (brokeUp && (ModeP == 0 || upIsReversal) && haveLo) { dir = GMDir.Long; sl = shLo - pad; }
            else if (brokeDn && (ModeP == 0 || dnIsReversal) && haveHi) { dir = GMDir.Short; sl = shHi + pad; }
            ...
            if (vol <= 0) { _log.Log("GM16", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM16_MS"))
            { _tradesToday++; _log.Log("GM16", "ENTRY", dir, entry, sl, tp, vol, RR, 1, ModeP == 0 ? "BOS" : "CHoCH"); }
        }
    }
}
