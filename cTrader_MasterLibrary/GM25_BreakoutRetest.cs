// GM25 Breakout-Retest — GODMODE Master cBot Library (cTrader)
// Range break, then enter on the retest that holds the broken level. Mirror of MT5 GM25.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM25_BreakoutRetest : Robot
    {
        ...
        [Parameter("Range Lookback", Group = "Entry", DefaultValue = 20)] public int RangeLB { get; set; }
        [Parameter("Retest Tol Pips", Group = "Entry", DefaultValue = 2.0)] public double RetestTolPips { get; set; }
        [Parameter("Max Wait Bars", Group = "Entry", DefaultValue = 8)] public int MaxWaitBars { get; set; }
        ...
        private bool _brkUp, _brkDn; private double _lvlUp, _lvlDn; private int _ageUp, _ageDn;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM25"); _guards = new GMGuards(this, "GM25");
            _log = new GMLogger("GODMODE_GM25_BreakoutRetest_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM25 Breakout-Retest started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM25", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            if (MaxConsec > 0 && _guards.TrailingLossStreak() >= MaxConsec) { _guards.Halt(); return; }
            if (H(now) >= ForceCloseHour) { _trade.CloseAll(); return; }

            double rh = GMStruct.HighestHigh(Bars, RangeLB, 2), rl = GMStruct.LowestLow(Bars, RangeLB, 2);
            double c1 = Bars.ClosePrices.Last(1), hi1 = Bars.HighPrices.Last(1), lo1 = Bars.LowPrices.Last(1);
            double pip = Symbol.PipSize, tol = RetestTolPips * pip, pad = SLPadPips * pip;

            // register fresh breakouts
            if (!_brkUp && c1 > rh) { _brkUp = true; _lvlUp = rh; _ageUp = 0; }
            else if (_brkUp) { _ageUp++; if (_ageUp > MaxWaitBars || c1 < _lvlUp - tol) _brkUp = false; }
            if (!_brkDn && c1 < rl) { _brkDn = true; _lvlDn = rl; _ageDn = 0; }
            else if (_brkDn) { _ageDn++; if (_ageDn > MaxWaitBars || c1 > _lvlDn + tol) _brkDn = false; }
            ...
            if (_brkUp && lo1 <= _lvlUp + tol && c1 > _lvlUp) { dir = GMDir.Long; sl = _lvlUp - pad; _brkUp = false; }
            else if (_brkDn && hi1 >= _lvlDn - tol && c1 < _lvlDn) { dir = GMDir.Short; sl = _lvlDn + pad; _brkDn = false; }
            ...
            if (vol <= 0) { _log.Log("GM25", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM25_BR"))
            { _tradesToday++; _log.Log("GM25", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "breakout-retest hold"); }
        }
    }
}
