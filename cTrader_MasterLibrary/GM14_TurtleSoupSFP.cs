// GM14 Turtle Soup / Swing Failure Pattern — GODMODE Master cBot Library (cTrader)
// Sweep of a prior swing then reclaim (Larry Connors Turtle Soup / ICT SFP). Mirror of MT5 GM14.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM14_TurtleSoupSFP : Robot
    {
        ...
        [Parameter("Swing Lookback", Group = "Entry", DefaultValue = 30)] public int SwingLB { get; set; }
        [Parameter("Swing Wing", Group = "Entry", DefaultValue = 2)] public int Wing { get; set; }
        [Parameter("Use EMA Bias", Group = "Entry", DefaultValue = false)] public bool UseEMA { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM14"); _guards = new GMGuards(this, "GM14");
            _log = new GMLogger("GODMODE_GM14_TurtleSoupSFP_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, EMAPeriod);
            Print("GM14 Turtle Soup / SFP started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM14", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double close = Bars.ClosePrices.Last(1), hi1 = Bars.HighPrices.Last(1), lo1 = Bars.LowPrices.Last(1);
            double ema = _ema.Result.Last(1);
            double pad = SLPadPips * Symbol.PipSize;
            double shHi, shLo; int sH, sL;
            bool haveHi = GMStruct.LastSwingHigh(Bars, SwingLB, Wing, out shHi, out sH);
            bool haveLo = GMStruct.LastSwingLow(Bars, SwingLB, Wing, out shLo, out sL);

            GMDir dir = GMDir.None; double sl = 0;
            // Bearish SFP: wick above a prior swing high, close back below it.
            if (haveHi && hi1 > shHi && close < shHi && (!UseEMA || close < ema))
            { dir = GMDir.Short; sl = hi1 + pad; }
            // Bullish SFP: wick below a prior swing low, close back above it.
            else if (haveLo && lo1 < shLo && close > shLo && (!UseEMA || close > ema))
            { dir = GMDir.Long; sl = lo1 - pad; }
            ...
            if (vol <= 0) { _log.Log("GM14", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM14_SFP"))
            { _tradesToday++; _log.Log("GM14", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "swing sweep + reclaim"); }
        }
    }
}
