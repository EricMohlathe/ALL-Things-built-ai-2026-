// GM26 Confluence Stack — GODMODE Master cBot Library (cTrader)
// The meta-edge: fire only when N independent layers agree (trend+FVG+zone+sweep+session+momentum).
// This is where the legitimately higher win-rate band lives (hard floor of 4-5 layers). Mirror of MT5 GM26.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM26_ConfluenceStack : Robot
    {
        ...
        [Parameter("Max MinLot Risk Mult", Group = "Risk", DefaultValue = 3.0)] public double MaxLotMultP { get; set; }
        ...
        [Parameter("Trade End Hour", Group = "Session", DefaultValue = 16)] public int TradeEndHour { get; set; }
        [Parameter("Force Close Hour", Group = "Session", DefaultValue = 22)] public int ForceCloseHour { get; set; }

        [Parameter("Min Layers", Group = "Confluence", DefaultValue = 4)] public int MinLayers { get; set; }
        [Parameter("Layer: Trend EMA", Group = "Confluence", DefaultValue = true)] public bool UseTrend { get; set; }
        [Parameter("Layer: FVG", Group = "Confluence", DefaultValue = true)] public bool UseFVG { get; set; }
        [Parameter("Layer: Zone (OB/Demand)", Group = "Confluence", DefaultValue = true)] public bool UseZone { get; set; }
        [Parameter("Layer: Swing Sweep", Group = "Confluence", DefaultValue = true)] public bool UseSweep { get; set; }
        [Parameter("Layer: Session", Group = "Confluence", DefaultValue = true)] public bool UseSessionLayer { get; set; }
        [Parameter("Layer: Momentum", Group = "Confluence", DefaultValue = true)] public bool UseMomentum { get; set; }

        [Parameter("EMA Period", Group = "Params", DefaultValue = 50)] public int EMAPeriod { get; set; }
        [Parameter("FVG Lookback", Group = "Params", DefaultValue = 15)] public int FVGLB { get; set; }
        [Parameter("Zone Lookback", Group = "Params", DefaultValue = 30)] public int ZoneLB { get; set; }
        [Parameter("Swing Lookback", Group = "Params", DefaultValue = 30)] public int SwingLB { get; set; }
        [Parameter("Swing Wing", Group = "Params", DefaultValue = 2)] public int Wing { get; set; }
        [Parameter("Displacement ATR Mult", Group = "Params", DefaultValue = 1.0)] public double DispMult { get; set; }
        [Parameter("Base Max ATR Mult", Group = "Params", DefaultValue = 0.6)] public double BaseMaxMult { get; set; }
        [Parameter("SL ATR Mult (fallback)", Group = "Params", DefaultValue = 1.5)] public double SLAtrMult { get; set; }
        [Parameter("SL Padding Pips", Group = "Params", DefaultValue = 1.0)] public double SLPadPips { get; set; }
        [Parameter("Reward:Risk", Group = "Params", DefaultValue = 2.0)] public double RR { get; set; }
        [Parameter("ATR Period", Group = "Params", DefaultValue = 14)] public int ATRPeriod { get; set; }
        [Parameter("Max Trades/Day", Group = "Params", DefaultValue = 3)] public int MaxTrades { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM26"); _guards = new GMGuards(this, "GM26");
            _log = new GMLogger("GODMODE_GM26_ConfluenceStack_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, EMAPeriod);
            Print("GM26 Confluence Stack started. MinLayers=" + MinLayers + " RiskMode=" + RiskModeP);
        }

        private double RiskPct() => GMUtil.ResolveRiskPct(RiskModeP, RiskCons, RiskAggr, RiskFlip);
        private double LotMult() => GMUtil.MaxLotMult(RiskModeP, MaxLotMultP);
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM26", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double o1 = Bars.OpenPrices.Last(1), c1 = Bars.ClosePrices.Last(1), hi1 = Bars.HighPrices.Last(1), lo1 = Bars.LowPrices.Last(1);
            double ema = _ema.Result.Last(1);
            bool inSess = GMUtil.InWindow(GMUtil.MinuteOfDay(now, OffsetHours), TradeStartHour * 60, TradeEndHour * 60);

            double ft, fb, zt, zb, swLo, swHi; int sLs, sHs;
            bool bullFvg = GMStruct.FindBullishFVG(Bars, FVGLB, out ft, out fb) && lo1 <= ft && c1 >= fb;
            bool bearFvg = GMStruct.FindBearishFVG(Bars, FVGLB, out ft, out fb) && hi1 >= fb && c1 <= ft;
            bool bullZone = (GMStruct.FindDemandBase(Bars, ZoneLB, atr, DispMult, BaseMaxMult, out zt, out zb) && lo1 <= zt && c1 >= zb)
                          || (GMStruct.FindBullishOB(Bars, ZoneLB, atr, DispMult, out zt, out zb) && lo1 <= zt && c1 >= zb);
            bool bearZone = (GMStruct.FindSupplyBase(Bars, ZoneLB, atr, DispMult, BaseMaxMult, out zt, out zb) && hi1 >= zb && c1 <= zt)
                          || (GMStruct.FindBearishOB(Bars, ZoneLB, atr, DispMult, out zt, out zb) && hi1 >= zb && c1 <= zt);
            bool haveLo = GMStruct.LastSwingLow(Bars, SwingLB, Wing, out swLo, out sLs);
            bool haveHi = GMStruct.LastSwingHigh(Bars, SwingLB, Wing, out swHi, out sHs);
            bool bullSweep = haveLo && lo1 < swLo && c1 > swLo;
            bool bearSweep = haveHi && hi1 > swHi && c1 < swHi;
            bool bullMom = (c1 - o1) > DispMult * atr || GMStruct.IsBullEngulf(Bars);
            bool bearMom = (o1 - c1) > DispMult * atr || GMStruct.IsBearEngulf(Bars);

            int longN = 0, shortN = 0;
            if (UseTrend) { if (c1 > ema) longN++; if (c1 < ema) shortN++; }
            if (UseFVG) { if (bullFvg) longN++; if (bearFvg) shortN++; }
            if (UseZone) { if (bullZone) longN++; if (bearZone) shortN++; }
            if (UseSweep) { if (bullSweep) longN++; if (bearSweep) shortN++; }
            if (UseSessionLayer && inSess) { longN++; shortN++; }
            if (UseMomentum) { if (bullMom) longN++; if (bearMom) shortN++; }

            GMDir dir = GMDir.None;
            if (longN >= MinLayers && longN > shortN) dir = GMDir.Long;
            else if (shortN >= MinLayers && shortN > longN) dir = GMDir.Short;
            if (dir == GMDir.None) return;

            double pad = SLPadPips * Symbol.PipSize;
            double sl = dir == GMDir.Long
                ? (haveLo ? swLo - pad : c1 - SLAtrMult * atr)
                : (haveHi ? swHi + pad : c1 + SLAtrMult * atr);

            double entry = dir == GMDir.Long ? Symbol.Ask : Symbol.Bid;
            double risk = Math.Abs(entry - sl); if (risk <= 0) return;
            double tp = dir == GMDir.Long ? entry + risk * RR : entry - risk * RR;

            double vol = GMUtil.CalcVolume(this, risk, RiskPct(), LotMult());
            int score = dir == GMDir.Long ? longN : shortN;
            if (vol <= 0) { _log.Log("GM26", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, score, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM26_Stack"))
            { _tradesToday++; _log.Log("GM26", "ENTRY", dir, entry, sl, tp, vol, RR, score, "confluence layers=" + score); }
        }
    }
}
