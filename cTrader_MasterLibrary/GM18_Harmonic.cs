// GM18 Harmonic patterns (XABCD) — GODMODE Master cBot Library (cTrader)
// Gartley / Bat / Butterfly / Crab via Fibonacci leg ratios on alternating fractal swings. Mirror of MT5 GM18.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM18_Harmonic : Robot
    {
        ...
        [Parameter("Swing Lookback", Group = "Entry", DefaultValue = 120)] public int SwingLB { get; set; }
        [Parameter("Swing Wing", Group = "Entry", DefaultValue = 3)] public int Wing { get; set; }
        [Parameter("Ratio Tolerance", Group = "Entry", DefaultValue = 0.09)] public double Tol { get; set; }
        [Parameter("Max D Age Bars", Group = "Entry", DefaultValue = 5)] public int MaxDBars { get; set; }
        [Parameter("PRZ ATR Mult", Group = "Entry", DefaultValue = 1.0)] public double PRZMult { get; set; }
        [Parameter("SL Padding Pips", Group = "Entry", DefaultValue = 2.0)] public double SLPadPips { get; set; }
        ...
        private AverageTrueRange _atr; private int _tradesToday;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM18"); _guards = new GMGuards(this, "GM18");
            _log = new GMLogger("GODMODE_GM18_Harmonic_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM18 Harmonic started. RiskMode=" + RiskModeP);
        }
        ...
        private static bool Near(double x, double t, double tol) => Math.Abs(x - t) <= tol;
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM18", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            // Collect up to 5 alternating fractal pivots, most-recent (D) first.
            double[] pp = new double[5]; int[] psh = new int[5]; bool[] phi = new bool[5]; int n = 0, lastType = 0;
            for (int s = Wing + 1; s <= SwingLB && n < 5; s++)
            {
                if (GMStruct.IsSwingHigh(Bars, s, Wing) && lastType != 1)
                { pp[n] = Bars.HighPrices.Last(s); psh[n] = s; phi[n] = true; lastType = 1; n++; }
                else if (GMStruct.IsSwingLow(Bars, s, Wing) && lastType != -1)
                { pp[n] = Bars.LowPrices.Last(s); psh[n] = s; phi[n] = false; lastType = -1; n++; }
            }
            if (n < 5) return;
            if (psh[0] > Wing + MaxDBars) return;   // D must be fresh

            double atr = Atr(); if (atr <= 0) return;
            double D = pp[0], C = pp[1], B = pp[2], A = pp[3], X = pp[4];
            double XA = Math.Abs(A - X), AB = Math.Abs(B - A), BC = Math.Abs(C - B), CD = Math.Abs(D - C), AD = Math.Abs(D - A);
            if (XA <= 0 || AB <= 0 || BC <= 0 || CD <= 0) return;
            double retAB = AB / XA, retBC = BC / AB, retAD = AD / XA;
            bool bcOK = retBC >= 0.382 - Tol && retBC <= 0.886 + Tol;
            bool gartley = Near(retAB, 0.618, Tol) && Near(retAD, 0.786, Tol);
            bool bat = retAB >= 0.382 - Tol && retAB <= 0.5 + Tol && Near(retAD, 0.886, Tol);
            bool bfly = Near(retAB, 0.786, Tol) && retAD >= 1.272 - Tol && retAD <= 1.618 + Tol;
            bool crab = retAB >= 0.382 - Tol && retAB <= 0.618 + Tol && Near(retAD, 1.618, Tol);
            if (!(bcOK && (gartley || bat || bfly || crab))) return;

            double close = Bars.ClosePrices.Last(1);
            if (Math.Abs(close - D) > PRZMult * atr) return;   // price must be at the PRZ

            double pad = SLPadPips * Symbol.PipSize;
            GMDir dir = phi[0] ? GMDir.Short : GMDir.Long;   // D high => bearish, D low => bullish
            double sl = dir == GMDir.Long ? Bars.LowPrices.Last(psh[0]) - pad : Bars.HighPrices.Last(psh[0]) + pad;
            ...
            if (vol <= 0) { _log.Log("GM18", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            string pat = gartley ? "Gartley" : bat ? "Bat" : bfly ? "Butterfly" : "Crab";
            if (_trade.Open(dir, vol, sl, tp, "GM18_" + pat))
            { _tradesToday++; _log.Log("GM18", "ENTRY", dir, entry, sl, tp, vol, RR, 1, pat + " PRZ"); }
        }
    }
}
