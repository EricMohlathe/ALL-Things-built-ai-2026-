// GM19 RSI(2) mean reversion — GODMODE Master cBot Library (cTrader)
// Connors RSI(2) pullback inside a long-EMA trend filter. Mirror of MT5 GM19.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM19_RSI2MeanReversion : Robot
    {
        ...
        [Parameter("RSI Period", Group = "Entry", DefaultValue = 2)] public int RsiPeriod { get; set; }
        [Parameter("RSI Oversold", Group = "Entry", DefaultValue = 10.0)] public double OS { get; set; }
        [Parameter("RSI Overbought", Group = "Entry", DefaultValue = 90.0)] public double OB { get; set; }
        [Parameter("Trend EMA Period", Group = "Entry", DefaultValue = 200)] public int TrendEMA { get; set; }
        [Parameter("SL ATR Mult", Group = "Entry", DefaultValue = 2.0)] public double SLAtrMult { get; set; }
        ...
        [Parameter("BE at R", Group = "Manage", DefaultValue = 0.0)] public double BEatR { get; set; }
        ...
        private AverageTrueRange _atr; private RelativeStrengthIndex _rsi; private ExponentialMovingAverage _ema; private int _tradesToday;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM19"); _guards = new GMGuards(this, "GM19");
            _log = new GMLogger("GODMODE_GM19_RSI2MeanReversion_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _rsi = Indicators.RelativeStrengthIndex(Bars.ClosePrices, RsiPeriod);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, TrendEMA);
            Print("GM19 RSI(2) mean reversion started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM19", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            if (close > ema && rsi < OS) { dir = GMDir.Long; sl = close - SLAtrMult * atr; }
            else if (close < ema && rsi > OB) { dir = GMDir.Short; sl = close + SLAtrMult * atr; }
            ...
            if (vol <= 0) { _log.Log("GM19", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM19_RSI2"))
            { _tradesToday++; _log.Log("GM19", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "RSI2 pullback in trend"); }
        }
    }
}
