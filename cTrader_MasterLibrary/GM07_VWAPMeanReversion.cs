// GM07 VWAP Mean Reversion (band fade) — GODMODE Master cBot Library (cTrader)
// AMT / Fabio Valentini / SMB VWAP reclaim. Mirror of MT5 GM07.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM07_VWAPMeanReversion : Robot
    {
        ...
        [Parameter("Trade Start Hour", Group = "Session", DefaultValue = 8)] public int TradeStartHour { get; set; }
        ...
        [Parameter("Band SD", Group = "Entry", DefaultValue = 2.0)] public double BandSD { get; set; }
        [Parameter("Require Rejection", Group = "Entry", DefaultValue = true)] public bool RequireRejection { get; set; }
        ...
        [Parameter("TP Type 0=VWAP 1=RR", Group = "Entry", DefaultValue = 0)] public int TPType { get; set; }
        [Parameter("Reward:Risk", Group = "Entry", DefaultValue = 1.5)] public double RR { get; set; }
        [Parameter("Min R:R", Group = "Entry", DefaultValue = 1.0)] public double MinRR { get; set; }
        ...
        [Parameter("Partial at R", Group = "Manage", DefaultValue = 0.0)] public double PartialAtR { get; set; }
        ...
        private double _cumPV, _cumV, _cumPV2, _vwap, _sd;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM07"); _guards = new GMGuards(this, "GM07");
            _log = new GMLogger("GODMODE_GM07_VWAP_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM07 VWAP Mean Reversion started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            var now = Server.Time;
            if (_guards.IsNewDay())
            { _guards.OnNewDay(); _tradesToday = 0; _cumPV = 0; _cumV = 0; _cumPV2 = 0; _vwap = 0; _sd = 0; }

            // update VWAP with just-closed bar
            double h = Bars.HighPrices.Last(1), l = Bars.LowPrices.Last(1), c = Bars.ClosePrices.Last(1), o = Bars.OpenPrices.Last(1);
            double typ = (h + l + c) / 3.0;
            double vol = Bars.TickVolumes.Last(1);
            if (vol > 0)
            {
                _cumPV += typ * vol; _cumV += vol; _cumPV2 += typ * typ * vol;
                if (_cumV > 0)
                {
                    _vwap = _cumPV / _cumV;
                    double var = (_cumPV2 / _cumV) - (_vwap * _vwap);
                    if (var < 0) var = 0;
                    _sd = Math.Sqrt(var);
                }
            }
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM07", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            if (_vwap <= 0 || _sd <= 0) return;

            double atr = Atr();
            double upper = _vwap + BandSD * _sd, lower = _vwap - BandSD * _sd;
            GMDir dir = GMDir.None; double sl = 0;

            if (c >= upper && (!RequireRejection || c < o)) { dir = GMDir.Short; sl = h + atr * SLATRMult; }
            if (dir == GMDir.None && c <= lower && (!RequireRejection || c > o)) { dir = GMDir.Long; sl = l - atr * SLATRMult; }
            if (dir == GMDir.None) return;

            double entry = dir == GMDir.Long ? Symbol.Ask : Symbol.Bid;
            double risk = Math.Abs(entry - sl);
            if (risk <= 0) return;
            double tp = TPType == 1 ? (dir == GMDir.Long ? entry + risk * RR : entry - risk * RR) : _vwap;
            double rr = Math.Abs(tp - entry) / risk;
            if (rr < MinRR) { _log.Log("GM07", "SKIP_RR", dir, entry, sl, tp, 0, rr, 0, "below MinRR"); return; }

            double v = GMUtil.CalcVolume(this, risk, RiskPct(), LotMult());
            if (v <= 0) { _log.Log("GM07", "SKIP_SIZE", dir, entry, sl, tp, 0, rr, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, v, sl, tp, "GM07_VWAP"))
            { _tradesToday++; _log.Log("GM07", "ENTRY", dir, entry, sl, tp, v, rr, 1, "VWAP band fade"); }
        }
    }
}
