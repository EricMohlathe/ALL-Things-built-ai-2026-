// GM10 Power of 3 / CRT with CISD — GODMODE Master cBot Library (cTrader)
// Trade-with-Pat PO3, Roboquant CRT, KISS (CISD). Mirror of MT5 GM10.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM10_PowerOfThree_CRT : Robot
    {
        ...
        [Parameter("Range Start Hour", Group = "Session", DefaultValue = 0)] public int RangeStartHour { get; set; }
        [Parameter("Range End Hour", Group = "Session", DefaultValue = 6)] public int RangeEndHour { get; set; }
        [Parameter("Trade Start Hour", Group = "Session", DefaultValue = 6)] public int TradeStartHour { get; set; }
        [Parameter("Trade End Hour", Group = "Session", DefaultValue = 12)] public int TradeEndHour { get; set; }
        [Parameter("Force Close Hour", Group = "Session", DefaultValue = 13)] public int ForceCloseHour { get; set; }

        [Parameter("Require CISD", Group = "Entry", DefaultValue = true)] public bool RequireCISD { get; set; }
        [Parameter("SL Padding Pips", Group = "Entry", DefaultValue = 1.0)] public double SLPadPips { get; set; }
        [Parameter("TP Type 0=OppEdge 1=SD 2=RR", Group = "Entry", DefaultValue = 0)] public int TPType { get; set; }
        [Parameter("SD Mult", Group = "Entry", DefaultValue = 1.0)] public double SDMult { get; set; }
        [Parameter("Reward:Risk", Group = "Entry", DefaultValue = 2.5)] public double RR { get; set; }
        [Parameter("Min R:R", Group = "Entry", DefaultValue = 1.5)] public double MinRR { get; set; }
        ...
        private GMTrade _trade; private GMGuards _guards; private GMLogger _log; private AverageTrueRange _atr;
        private double _rHigh, _rLow, _cisd, _manipExtreme; private bool _rangeReady, _tradedToday; private int _phase;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM10"); _guards = new GMGuards(this, "GM10");
            _log = new GMLogger("GODMODE_GM10_PO3_CRT_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM10 Power-of-3 / CRT started. RiskMode=" + RiskModeP);
        }
        ...
        private void ResetDay() { _rHigh = 0; _rLow = 0; _rangeReady = false; _tradedToday = false; _phase = 0; _cisd = 0; _manipExtreme = 0; }
        ...
        {
            var now = Server.Time;
            int mod = GMUtil.MinuteOfDay(now, OffsetHours);

            if (_guards.IsNewDay()) { _guards.OnNewDay(); ResetDay(); }
            if (_guards.IsHalted) return;
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM10", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            if (MaxConsec > 0 && _guards.TrailingLossStreak() >= MaxConsec) { _guards.Halt(); return; }
            if (H(now) >= ForceCloseHour) { _trade.CloseAll(); return; }

            double prevHigh = Bars.HighPrices.Last(1), prevLow = Bars.LowPrices.Last(1);
            bool inRange = GMUtil.InWindow(mod, RangeStartHour * 60, RangeEndHour * 60);
            if (inRange)
            {
                if (_rHigh == 0 || prevHigh > _rHigh) _rHigh = prevHigh;
                if (_rLow == 0 || prevLow < _rLow) _rLow = prevLow;
            }
            else if (_rHigh > 0 && _rLow > 0 && !_rangeReady && mod >= RangeEndHour * 60)
            {
                _rangeReady = true;
                Chart.DrawHorizontalLine("GM10_RH", _rHigh, Color.Gold);
                Chart.DrawHorizontalLine("GM10_RL", _rLow, Color.Gold);
            }

            if (!_rangeReady || _tradedToday) return;
            if (!GMUtil.InWindow(mod, TradeStartHour * 60, TradeEndHour * 60)) return;
            if (_trade.HasPosition) return;

            double barHigh = Bars.HighPrices.Last(1), barLow = Bars.LowPrices.Last(1), close = Bars.ClosePrices.Last(1);

            if (_phase == 0)
            {
                if (barHigh > _rHigh) { _phase = 1; _manipExtreme = barHigh; _cisd = barLow; }
                else if (barLow < _rLow) { _phase = -1; _manipExtreme = barLow; _cisd = barHigh; }
                return;
            }
            if (_phase == 1 && barHigh > _manipExtreme) { _manipExtreme = barHigh; _cisd = barLow; }
            if (_phase == -1 && barLow < _manipExtreme) { _manipExtreme = barLow; _cisd = barHigh; }

            GMDir dir = GMDir.None;
            if (_phase == 1 && (!RequireCISD || close < _cisd)) dir = GMDir.Short;
            if (_phase == -1 && (!RequireCISD || close > _cisd)) dir = GMDir.Long;
            ...
            double sl = dir == GMDir.Long ? _manipExtreme - pad : _manipExtreme + pad;
            double risk = Math.Abs(entry - sl); if (risk <= 0) return;
            double rangeSize = _rHigh - _rLow;
            double tp;
            if (TPType == 2) tp = dir == GMDir.Long ? entry + risk * RR : entry - risk * RR;
            else if (TPType == 1) tp = dir == GMDir.Long ? GMStruct.SDExtension(_rHigh, rangeSize, 1, SDMult)
                                                         : GMStruct.SDExtension(_rLow, rangeSize, -1, SDMult);
            else tp = dir == GMDir.Long ? _rHigh : _rLow;

            double rr = Math.Abs(tp - entry) / risk;
            if (rr < MinRR) { _log.Log("GM10", "SKIP_RR", dir, entry, sl, tp, 0, rr, 0, "below MinRR"); return; }

            double vol = GMUtil.CalcVolume(this, risk, RiskPct(), LotMult());
            if (vol <= 0) { _log.Log("GM10", "SKIP_SIZE", dir, entry, sl, tp, 0, rr, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM10_PO3"))
            { _tradedToday = true; _log.Log("GM10", "ENTRY", dir, entry, sl, tp, vol, rr, 2, _phase == 1 ? "manip high->short(CISD)" : "manip low->long(CISD)"); }
        }
    }
}
