// GM03 FVG mitigation entry (trend-aligned) — GODMODE Master cBot Library (cTrader)
// ICT FVG / Casper FCR-FVG. Mirror of MT5 GM03.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM03_FVGEntry : Robot
    {
        ...
        [Parameter("FVG Lookback", Group = "Entry", DefaultValue = 20)] public int FVGLookback { get; set; }
        [Parameter("Min Gap Pips", Group = "Entry", DefaultValue = 2.0)] public double MinGapPips { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM03"); _guards = new GMGuards(this, "GM03");
            _log = new GMLogger("GODMODE_GM03_FVG_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, EMAPeriod);
            Print("GM03 FVG started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM03", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            if (GMStruct.FindBullishFVG(Bars, FVGLookback, out top, out bot) && (top - bot) >= minGap)
            {
                if (price <= top && price >= bot && (!UseEMA || price > ema)) { dir = GMDir.Long; sl = bot - pad; }
            }
            if (dir == GMDir.None && GMStruct.FindBearishFVG(Bars, FVGLookback, out top, out bot) && (top - bot) >= minGap)
            {
                if (price <= top && price >= bot && (!UseEMA || price < ema)) { dir = GMDir.Short; sl = top + pad; }
            }
            ...
            if (vol <= 0) { _log.Log("GM03", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM03_FVG"))
            { _tradesToday++; _log.Log("GM03", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "FVG mitigation"); }
        }
    }
}
