// GM24 SMT divergence — GODMODE Master cBot Library (cTrader)
// ICT Smart-Money Technique: correlated pair fails to confirm a new high/low. Mirror of MT5 GM24.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM24_SMTDivergence : Robot
    {
        ...
        [Parameter("Correlated Symbol", Group = "Entry", DefaultValue = "GBPUSD")] public string OtherSymbol { get; set; }
        [Parameter("Positive Correlation", Group = "Entry", DefaultValue = true)] public bool PosCorr { get; set; }
        [Parameter("Window Bars", Group = "Entry", DefaultValue = 12)] public int Win { get; set; }
        ...
        private AverageTrueRange _atr; private Bars _other; private int _tradesToday;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM24"); _guards = new GMGuards(this, "GM24");
            _log = new GMLogger("GODMODE_GM24_SMTDivergence_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _other = MarketData.GetBars(Bars.TimeFrame, OtherSymbol);
            Print("GM24 SMT Divergence started vs " + OtherSymbol + ". RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM24", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            if (_other == null || _other.ClosePrices.Count < 2 * Win + 2) return;

            // near = [1..Win], far = [Win+1..2Win] on both instruments.
            double tNearHi = GMStruct.HighestHigh(Bars, Win, 1), tFarHi = GMStruct.HighestHigh(Bars, Win, Win + 1);
            double tNearLo = GMStruct.LowestLow(Bars, Win, 1), tFarLo = GMStruct.LowestLow(Bars, Win, Win + 1);
            double oNearHi = GMStruct.HighestHigh(_other, Win, 1), oFarHi = GMStruct.HighestHigh(_other, Win, Win + 1);
            double oNearLo = GMStruct.LowestLow(_other, Win, 1), oFarLo = GMStruct.LowestLow(_other, Win, Win + 1);

            double o1 = Bars.OpenPrices.Last(1), c1 = Bars.ClosePrices.Last(1);
            double pad = SLPadPips * Symbol.PipSize;
            GMDir dir = GMDir.None; double sl = 0;

            // Bearish SMT: this makes a higher high, correlate fails (lower/equal high). Confirm with a down close.
            bool otherFailHigh = PosCorr ? (oNearHi <= oFarHi) : (oNearLo >= oFarLo);
            bool otherFailLow = PosCorr ? (oNearLo >= oFarLo) : (oNearHi <= oFarHi);
            if (tNearHi > tFarHi && otherFailHigh && c1 < o1) { dir = GMDir.Short; sl = tNearHi + pad; }
            else if (tNearLo < tFarLo && otherFailLow && c1 > o1) { dir = GMDir.Long; sl = tNearLo - pad; }
            ...
            if (vol <= 0) { _log.Log("GM24", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM24_SMT"))
            { _tradesToday++; _log.Log("GM24", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "SMT vs " + OtherSymbol); }
        }
    }
}
