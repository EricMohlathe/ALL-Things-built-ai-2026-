// GM02 Session Sweep -> Reclaim reversal — GODMODE Master cBot Library (cTrader)
// ICT Judas / TJR Asia Sweep / CRT / Power-of-3. Mirror of MT5 GM02.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM02_SessionSweepReversal : Robot
    {
        ...
        [Parameter("Trade End Hour", Group = "Session", DefaultValue = 11)] public int TradeEndHour { get; set; }
        [Parameter("Force Close Hour", Group = "Session", DefaultValue = 12)] public int ForceCloseHour { get; set; }

        [Parameter("Require Reclaim", Group = "Entry", DefaultValue = true)] public bool RequireReclaim { get; set; }
        [Parameter("SL Type 0=Swept 1=ATR", Group = "Entry", DefaultValue = 0)] public int SLType { get; set; }
        [Parameter("ATR Mult SL", Group = "Entry", DefaultValue = 1.5)] public double ATRMultSL { get; set; }
        ...
        [Parameter("TP Type 0=OppEdge 1=RR", Group = "Entry", DefaultValue = 0)] public int TPType { get; set; }
        ...
        private double _rHigh, _rLow, _sweepExtreme; private bool _rangeReady, _sweptHigh, _sweptLow, _tradedToday;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM02"); _guards = new GMGuards(this, "GM02");
            _log = new GMLogger("GODMODE_GM02_SweepReversal_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM02 Session Sweep Reversal started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            int rStart = RangeStartHour * 60, rEnd = RangeEndHour * 60;

            if (_guards.IsNewDay())
            { _guards.OnNewDay(); _rHigh = 0; _rLow = 0; _rangeReady = false; _sweptHigh = false; _sweptLow = false; _sweepExtreme = 0; _tradedToday = false; }
            if (_guards.IsHalted) return;
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM02", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            bool inRange = GMUtil.InWindow(mod, rStart, rEnd);
            if (inRange)
            {
                if (_rHigh == 0 || prevHigh > _rHigh) _rHigh = prevHigh;
                if (_rLow == 0 || prevLow < _rLow) _rLow = prevLow;
            }
            else if (_rHigh > 0 && _rLow > 0 && !_rangeReady && mod >= rEnd)
            {
                _rangeReady = true;
                Chart.DrawHorizontalLine("GM02_RH", _rHigh, Color.Aqua);
                Chart.DrawHorizontalLine("GM02_RL", _rLow, Color.Aqua);
            }

            if (!_rangeReady || _tradedToday || _trade.HasPosition) return;
            ...
            if (!_sweptHigh && !_sweptLow)
            {
                if (barHigh > _rHigh) { _sweptHigh = true; _sweepExtreme = barHigh; }
                else if (barLow < _rLow) { _sweptLow = true; _sweepExtreme = barLow; }
            }
            else if (_sweptHigh && barHigh > _sweepExtreme) _sweepExtreme = barHigh;
            else if (_sweptLow && barLow < _sweepExtreme) _sweepExtreme = barLow;
            if (!_sweptHigh && !_sweptLow) return;

            GMDir dir = GMDir.None;
            if (_sweptHigh && (!RequireReclaim || close < _rHigh)) dir = GMDir.Short;
            else if (_sweptLow && (!RequireReclaim || close > _rLow)) dir = GMDir.Long;
            ...
            double atr = Atr();
            double sl = (SLType == 1 && atr > 0)
                ? (dir == GMDir.Long ? entry - atr * ATRMultSL : entry + atr * ATRMultSL)
                : (dir == GMDir.Long ? _sweepExtreme - pad : _sweepExtreme + pad);
            double risk = Math.Abs(entry - sl);
            if (risk <= 0) return;
            double tp = TPType == 1 ? (dir == GMDir.Long ? entry + risk * RR : entry - risk * RR)
                                    : (dir == GMDir.Long ? _rHigh : _rLow);
            double rr = Math.Abs(tp - entry) / risk;
            if (rr < MinRR) { _log.Log("GM02", "SKIP_RR", dir, entry, sl, tp, 0, rr, 0, "below MinRR"); return; }

            double vol = GMUtil.CalcVolume(this, risk, RiskPct(), LotMult());
            if (vol <= 0) { _log.Log("GM02", "SKIP_SIZE", dir, entry, sl, tp, 0, rr, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM02_SweepRev"))
            { _tradedToday = true; _log.Log("GM02", "ENTRY", dir, entry, sl, tp, vol, rr, 1, _sweptHigh ? "high swept->short" : "low swept->long"); }
        }
    }
}
