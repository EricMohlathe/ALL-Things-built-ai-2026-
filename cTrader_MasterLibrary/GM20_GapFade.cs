// GM20 Gap fill / Gap-and-Go — GODMODE Master cBot Library (cTrader)
// Opening-gap vs prior daily close: fade back to close, or trade continuation. Mirror of MT5 GM20.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM20_GapFade : Robot
    {
        ...
        [Parameter("Session Start Hour", Group = "Session", DefaultValue = 8)] public int SessStartHour { get; set; }
        [Parameter("Entry Window Hours", Group = "Session", DefaultValue = 3)] public int WindowHours { get; set; }
        [Parameter("Force Close Hour", Group = "Session", DefaultValue = 22)] public int ForceCloseHour { get; set; }

        [Parameter("Mode 0=Fade 1=GapGo", Group = "Entry", DefaultValue = 0)] public int ModeP { get; set; }
        [Parameter("Min Gap ATR Mult", Group = "Entry", DefaultValue = 0.8)] public double GapAtrMult { get; set; }
        [Parameter("SL ATR Mult", Group = "Entry", DefaultValue = 1.5)] public double SLAtrMult { get; set; }
        [Parameter("Reward:Risk (GapGo)", Group = "Entry", DefaultValue = 2.0)] public double RR { get; set; }
        [Parameter("ATR Period", Group = "Entry", DefaultValue = 14)] public int ATRPeriod { get; set; }
        [Parameter("Max Trades/Day", Group = "Entry", DefaultValue = 1)] public int MaxTrades { get; set; }
        ...
        private AverageTrueRange _atr; private Bars _daily; private int _tradesToday;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM20"); _guards = new GMGuards(this, "GM20");
            _log = new GMLogger("GODMODE_GM20_GapFade_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _daily = MarketData.GetBars(TimeFrame.Daily);
            Print("GM20 Gap Fade/Go started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM20", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            if (!GMUtil.InWindow(mod, SessStartHour * 60, (SessStartHour + WindowHours) * 60)) return;
            if (_trade.HasPosition || _tradesToday >= MaxTrades) return;
            if (!_guards.SpreadOK(MaxSpread)) return;

            double atr = Atr(); if (atr <= 0 || _daily.ClosePrices.Count < 2) return;
            double yClose = _daily.ClosePrices.Last(1);
            double tOpen = _daily.OpenPrices.LastValue;        // current day open
            double gap = tOpen - yClose;
            double thr = GapAtrMult * atr;
            if (Math.Abs(gap) < thr) return;

            GMDir dir = GMDir.None; double sl = 0, tp = 0;
            double entry = gap > 0 ? Symbol.Bid : Symbol.Ask;   // tentative; set per dir below

            if (ModeP == 0)   // Fade back toward prior close
            {
                if (gap > 0) { dir = GMDir.Short; entry = Symbol.Bid; sl = entry + SLAtrMult * atr; tp = yClose; }
                else { dir = GMDir.Long; entry = Symbol.Ask; sl = entry - SLAtrMult * atr; tp = yClose; }
            }
            else              // Gap-and-Go continuation
            {
                if (gap > 0) { dir = GMDir.Long; entry = Symbol.Ask; sl = entry - SLAtrMult * atr; }
                else { dir = GMDir.Short; entry = Symbol.Bid; sl = entry + SLAtrMult * atr; }
            }
            if (dir == GMDir.None) return;
            double risk = Math.Abs(entry - sl); if (risk <= 0) return;
            if (ModeP == 1) tp = dir == GMDir.Long ? entry + risk * RR : entry - risk * RR;
            double rr = Math.Abs(tp - entry) / risk;
            if (rr <= 0) { _log.Log("GM20", "SKIP_RR", dir, entry, sl, tp, 0, 0, 0, "tp wrong side"); return; }

            double vol = GMUtil.CalcVolume(this, risk, RiskPct(), LotMult());
            if (vol <= 0) { _log.Log("GM20", "SKIP_SIZE", dir, entry, sl, tp, 0, rr, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, ModeP == 0 ? "GM20_Fade" : "GM20_GapGo"))
            { _tradesToday++; _log.Log("GM20", "ENTRY", dir, entry, sl, tp, vol, rr, 1, ModeP == 0 ? "gap fade" : "gap go"); }
        }
    }
}
