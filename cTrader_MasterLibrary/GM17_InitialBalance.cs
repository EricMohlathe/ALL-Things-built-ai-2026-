// GM17 Initial Balance breakout — GODMODE Master cBot Library (cTrader)
// First-N-minutes range; trade the break of IB high/low (market-profile IB extension). Mirror of MT5 GM17.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM17_InitialBalance : Robot
    {
        ...
        [Parameter("IB Start Hour", Group = "Session", DefaultValue = 8)] public int IBStartHour { get; set; }
        [Parameter("IB End Hour", Group = "Session", DefaultValue = 9)] public int IBEndHour { get; set; }
        ...
        [Parameter("SL = opposite IB edge", Group = "Entry", DefaultValue = false)] public bool SLOppEdge { get; set; }
        ...
        private double _ibHi, _ibLo; private bool _ibValid;

        protected override void OnStart()
        {
            _trade = new GMTrade(this, "GM17"); _guards = new GMGuards(this, "GM17");
            _log = new GMLogger("GODMODE_GM17_InitialBalance_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM17 Initial Balance started. RiskMode=" + RiskModeP);
        }
        ...
        {
            var now = Server.Time;
            if (_guards.IsNewDay()) { _guards.OnNewDay(); _tradesToday = 0; _ibHi = double.MinValue; _ibLo = double.MaxValue; _ibValid = false; }
            if (_guards.IsHalted) return;
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM17", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double hi1 = Bars.HighPrices.Last(1), lo1 = Bars.LowPrices.Last(1);

            // Accumulate the IB range during [IBStart, IBEnd).
            if (GMUtil.InWindow(mod, IBStartHour * 60, IBEndHour * 60))
            {
                if (hi1 > _ibHi) _ibHi = hi1;
                if (lo1 < _ibLo) _ibLo = lo1;
                _ibValid = _ibHi > _ibLo;
                return;
            }
            if (!_ibValid) return;
            if (!GMUtil.InWindow(mod, IBEndHour * 60, TradeEndHour * 60)) return;
            ...
            if (close > _ibHi) { dir = GMDir.Long; sl = SLOppEdge ? _ibLo - pad : lo1 - pad; }
            else if (close < _ibLo) { dir = GMDir.Short; sl = SLOppEdge ? _ibHi + pad : hi1 + pad; }
            ...
            if (vol <= 0) { _log.Log("GM17", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM17_IB"))
            { _tradesToday++; _log.Log("GM17", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "IB breakout"); }
        }
    }
}
