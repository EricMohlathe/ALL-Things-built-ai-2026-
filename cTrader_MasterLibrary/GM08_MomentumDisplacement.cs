// GM08 Momentum / Displacement continuation — GODMODE Master cBot Library (cTrader)
// No-Wick / displacement, Velez / Teo trend. Mirror of MT5 GM08.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM08_MomentumDisplacement : Robot
    {
        ...
        [Parameter("Range xATR", Group = "Entry", DefaultValue = 1.5)] public double RangeATRMult { get; set; }
        [Parameter("Min Body %", Group = "Entry", DefaultValue = 0.6)] public double MinBodyPct { get; set; }
        [Parameter("Max Opposing Wick %", Group = "Entry", DefaultValue = 0.25)] public double MaxWickPct { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM08"); _guards = new GMGuards(this, "GM08");
            _log = new GMLogger("GODMODE_GM08_Momentum_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _ema = Indicators.ExponentialMovingAverage(Bars.ClosePrices, EMAPeriod);
            Print("GM08 Momentum/Displacement started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM08", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            if (!(range >= RangeATRMult * atr && body / range >= MinBodyPct)) return;

            double topWick = h - Math.Max(o, c), botWick = Math.Min(o, c) - l;
            GMDir dir = GMDir.None;
            if (c > o && botWick / range <= MaxWickPct) dir = GMDir.Long;
            else if (c < o && topWick / range <= MaxWickPct) dir = GMDir.Short;
            if (dir == GMDir.None) return;
            if (UseEMA && ((dir == GMDir.Long && c <= ema) || (dir == GMDir.Short && c >= ema))) return;

            double entry = dir == GMDir.Long ? Symbol.Ask : Symbol.Bid;
            double sl = dir == GMDir.Long ? entry - atr * SLATRMult : entry + atr * SLATRMult;
            double risk = Math.Abs(entry - sl);
            if (risk <= 0) return;
            double tp = dir == GMDir.Long ? entry + risk * RR : entry - risk * RR;

            double vol = GMUtil.CalcVolume(this, risk, RiskPct(), LotMult());
            if (vol <= 0) { _log.Log("GM08", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM08_Momentum"))
            { _tradesToday++; _log.Log("GM08", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "displacement continuation"); }
        }
    }
}
