// GM23 New-Day Opening Gap / Consequent Encroachment — GODMODE Master cBot Library (cTrader)
// ICT NDOG/NWOG: trade the reaction at the gap midpoint (CE) in the gap direction. Mirror of MT5 GM23.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM23_OpeningGapCE : Robot
    {
        ...
        [Parameter("Min Gap ATR Mult", Group = "Entry", DefaultValue = 0.5)] public double MinGapMult { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM23"); _guards = new GMGuards(this, "GM23");
            _log = new GMLogger("GODMODE_GM23_OpeningGapCE_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            _daily = MarketData.GetBars(TimeFrame.Daily);
            Print("GM23 Opening Gap CE started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM23", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double tOpen = _daily.OpenPrices.LastValue;
            double gap = tOpen - yClose;
            if (Math.Abs(gap) < MinGapMult * atr) return;
            double ce = (yClose + tOpen) / 2.0;
            ...
            if (gap > 0 && lo1 <= ce && close >= ce) { dir = GMDir.Long; sl = Math.Min(yClose, lo1) - pad; }
            else if (gap < 0 && hi1 >= ce && close <= ce) { dir = GMDir.Short; sl = Math.Max(yClose, hi1) + pad; }
            ...
            if (vol <= 0) { _log.Log("GM23", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM23_CE"))
            { _tradesToday++; _log.Log("GM23", "ENTRY", dir, entry, sl, tp, vol, RR, 1, "opening-gap CE react"); }
        }
    }
}
