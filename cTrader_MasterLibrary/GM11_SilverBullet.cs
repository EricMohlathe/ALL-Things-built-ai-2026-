// GM11 ICT Silver Bullet — GODMODE Master cBot Library (cTrader)
// Macro time window + liquidity sweep + FVG. Mirror of MT5 GM11.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM11_SilverBullet : Robot
    {
        ...
        [Parameter("Offset Hours", Group = "Windows", DefaultValue = 0)] public int OffsetHours { get; set; }
        [Parameter("W1 Enable", Group = "Windows", DefaultValue = true)] public bool W1En { get; set; }
        [Parameter("W1 Start", Group = "Windows", DefaultValue = 10)] public int W1S { get; set; }
        [Parameter("W1 End", Group = "Windows", DefaultValue = 11)] public int W1E { get; set; }
        [Parameter("W2 Enable", Group = "Windows", DefaultValue = true)] public bool W2En { get; set; }
        [Parameter("W2 Start", Group = "Windows", DefaultValue = 14)] public int W2S { get; set; }
        [Parameter("W2 End", Group = "Windows", DefaultValue = 15)] public int W2E { get; set; }
        [Parameter("W3 Enable", Group = "Windows", DefaultValue = false)] public bool W3En { get; set; }
        [Parameter("W3 Start", Group = "Windows", DefaultValue = 3)] public int W3S { get; set; }
        [Parameter("W3 End", Group = "Windows", DefaultValue = 4)] public int W3E { get; set; }
        [Parameter("Force Close Hour", Group = "Windows", DefaultValue = 22)] public int ForceCloseHour { get; set; }

        [Parameter("Sweep Lookback", Group = "Entry", DefaultValue = 12)] public int SweepLB { get; set; }
        [Parameter("FVG Lookback", Group = "Entry", DefaultValue = 8)] public int FVGLB { get; set; }
        [Parameter("Min Gap Pips", Group = "Entry", DefaultValue = 1.5)] public double MinGapPips { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM11"); _guards = new GMGuards(this, "GM11");
            _log = new GMLogger("GODMODE_GM11_SilverBullet_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM11 Silver Bullet started. RiskMode=" + RiskModeP);
        }
        ...
        private bool InAnyWindow(int mod)
            => (W1En && GMUtil.InWindow(mod, W1S * 60, W1E * 60))
            || (W2En && GMUtil.InWindow(mod, W2S * 60, W2E * 60))
            || (W3En && GMUtil.InWindow(mod, W3S * 60, W3E * 60));
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM11", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double minGap = MinGapPips * Symbol.PipSize;
            double pad = SLPadPips * Symbol.PipSize;
            double price = Bars.ClosePrices.Last(1);
            double priorLow = GMStruct.LowestLow(Bars, SweepLB, 2);
            double priorHigh = GMStruct.HighestHigh(Bars, SweepLB, 2);
            double curLow = Bars.LowPrices.Last(1), curHigh = Bars.HighPrices.Last(1);
            bool sweptLow = curLow < priorLow;
            bool sweptHigh = curHigh > priorHigh;

            GMDir dir = GMDir.None; double sl = 0; double top, bot;
            if (sweptLow && GMStruct.FindBullishFVG(Bars, FVGLB, out top, out bot) && (top - bot) >= minGap)
            {
                if (price <= top && price >= bot) { dir = GMDir.Long; sl = curLow - pad; }
            }
            if (dir == GMDir.None && sweptHigh && GMStruct.FindBearishFVG(Bars, FVGLB, out top, out bot) && (top - bot) >= minGap)
            {
                if (price <= top && price >= bot) { dir = GMDir.Short; sl = curHigh + pad; }
            }
            ...
            if (vol <= 0) { _log.Log("GM11", "SKIP_SIZE", dir, entry, sl, tp, 0, RR, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM11_SB"))
            { _tradesToday++; _log.Log("GM11", "ENTRY", dir, entry, sl, tp, vol, RR, 2, "silver bullet sweep+FVG"); }
        }
    }
}
