// GM06 Wyckoff Spring / Upthrust — GODMODE Master cBot Library (cTrader)
// Wyckoff 2.0, GODMODE setups 14/15. Mirror of MT5 GM06.
using System;
using cAlgo.API;
...
namespace cAlgo.Robots
{
    ...
    public class GM06_WyckoffSpring : Robot
    {
        ...
        [Parameter("Range Length", Group = "Entry", DefaultValue = 20)] public int RangeLen { get; set; }
        [Parameter("Range Start Shift", Group = "Entry", DefaultValue = 2)] public int RangeStartShift { get; set; }
        [Parameter("Max Range xATR", Group = "Entry", DefaultValue = 4.0)] public double MaxRangeATR { get; set; }
        ...
        {
            _trade = new GMTrade(this, "GM06"); _guards = new GMGuards(this, "GM06");
            _log = new GMLogger("GODMODE_GM06_Wyckoff_log.csv");
            _atr = Indicators.AverageTrueRange(ATRPeriod, MovingAverageType.Simple);
            Print("GM06 Wyckoff Spring/Upthrust started. RiskMode=" + RiskModeP);
        }
        ...
        protected override void OnBar()
        {
            ...
            if (_guards.DailyDDBreached(MaxDD)) { _trade.CloseAll(); _guards.Halt(); _log.Log("GM06", "KILL_DD", GMDir.None, 0, 0, 0, 0, 0, 0, "daily DD"); return; }
            ...
            double atr = Atr();
            if (atr <= 0) return;
            if (Bars.Count < RangeLen + RangeStartShift + 2) return;

            double rangeHigh = GMStruct.HighestHigh(Bars, RangeLen, RangeStartShift);
            double rangeLow = GMStruct.LowestLow(Bars, RangeLen, RangeStartShift);
            double width = rangeHigh - rangeLow;
            if (width <= 0 || width > MaxRangeATR * atr) return;

            double pad = SLPadPips * Symbol.PipSize;
            double low1 = Bars.LowPrices.Last(1), high1 = Bars.HighPrices.Last(1), close1 = Bars.ClosePrices.Last(1);

            GMDir dir = GMDir.None; double sl = 0;
            bool spring = low1 < rangeLow && close1 > rangeLow;
            bool upthrust = high1 > rangeHigh && close1 < rangeHigh;
            if (spring) { dir = GMDir.Long; sl = low1 - pad; }
            else if (upthrust) { dir = GMDir.Short; sl = high1 + pad; }
            ...
            double tp = TPType == 1 ? (dir == GMDir.Long ? entry + risk * RR : entry - risk * RR)
                                    : (dir == GMDir.Long ? rangeHigh : rangeLow);
            double rr = Math.Abs(tp - entry) / risk;
            if (rr < MinRR) { _log.Log("GM06", "SKIP_RR", dir, entry, sl, tp, 0, rr, 0, "below MinRR"); return; }

            double vol = GMUtil.CalcVolume(this, risk, RiskPct(), LotMult());
            if (vol <= 0) { _log.Log("GM06", "SKIP_SIZE", dir, entry, sl, tp, 0, rr, 0, "min lot risk too high"); return; }

            if (_trade.Open(dir, vol, sl, tp, "GM06_Wyckoff"))
            { _tradesToday++; _log.Log("GM06", "ENTRY", dir, entry, sl, tp, vol, rr, 1, spring ? "spring" : "upthrust"); }
        }
    }
}
